...
from sqlalchemy import JSON, BigInteger, Column, Numeric, String, Boolean, DateTime, Text, ForeignKey
            ...
            with get_db() as db:
                ...
                if not subscription:
                    ...
                    return {
                        ...
                        "subscription": {
                            "user_id": user_id,
                            "plan_id": "free",
                            "plan": PlanModel.model_validate(free_plan).model_dump() if free_plan else None,
                            "status": "active",
                            "is_subscribed": False
                        }
                    }
                ...
                return {
                    ...
                    "subscription": {
                        ...
                        "plan": PlanModel.model_validate(plan).model_dump() if plan else None,
    ...
    def get_redeem_code(self, code: str) -> Optional[RedeemCodeModel]:
        try:
            with get_db() as db:
                ...
                return RedeemCodeModel.model_validate(redeem_code) if redeem_code else None
        except Exception:
            return None
        ...
        try:
            with get_db() as db:
                redeem_code = db.query(RedeemCode).filter(
                    ...
                    (RedeemCode.expires_at == None) | (RedeemCode.expires_at >= int(time.time()))
                ).first()
...
class PaymentsTable:
    ...
    def update_payment_status(self, payment_id: str, status: str, transaction_id: Optional[str] = None) -> Optional[PaymentModel]:
        try:
            update_data = {
                "status": status,
    ...
    def get_redeem_codes(self, page: int = 1, limit: int = 10) -> Dict[str, Any]:
        ...
        try:
            with get_db() as db:
                ...
                codes = db.query(RedeemCode).order_by(RedeemCode.created_at.desc()).offset((page - 1) * limit).limit(limit).all()
                ...
                return {
                    ...
                    "page": page,
                    "limit": limit,
                    "codes": [RedeemCodeModel.model_validate(code).model_dump() for code in codes]
                }
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))
...
class PaymentsTable:
                ...
                if status == "completed":
                    ...
                    if existing_subscription:
                        ...
                        if existing_subscription.plan_id == payment.plan_id:
                            # 同一套餐，直接延长时间
                            existing_subscription.end_date = existing_subscription.end_date + 30 * 86400
                        else:
                            # 不同套餐，更新套餐并重置时间
    ...
    def get_payments(self, user_id: Optional[str] = None, page: int = 1, limit: int = 10) -> Dict[str, Any]:
        """获取支付记录列表"""
        try:
            with get_db() as db:
